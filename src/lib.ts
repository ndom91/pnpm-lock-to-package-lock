...
import { getLockfileImporterId, type Lockfile, type ProjectSnapshot, readWantedLockfile } from "@pnpm/lockfile-file"
...
export async function parseLockfile(pkgPath: string) {
  ...
  if (Number.parseFloat(lock.lockfileVersion) > LATEST_SUPPORTED_PNPM_LOCK_VERSION)
    console.warn(
